#!/usr/bin/env python3
"""
Fast generator for entropy maps + label matrices.

Two changes vs generate_entropy_maps.py:

  1. H0-only shortcut: the H0 death times of a Rips filtration on n points are
     exactly the edge weights of the Euclidean MST (single-linkage merge weights).
     We replace 48 Ripser calls per map with 48 scipy MST calls on prefix slices
     of a single precomputed pairwise distance matrix. Outputs are numerically
     identical up to floating-point tolerance.

  2. Process-pool parallelism: each map is independent, so we fan out via
     multiprocessing.Pool. FeaturesGenerator is built once per worker in the
     initializer to amortize setup. Results stream back to the main process
     which writes memmaps and metadata.csv.

Combined speedup on a typical many-core EC2 instance is 100-1000x vs the
sequential Ripser version.
"""
...
def _build_pool(selections, fv, query_embed, rng):
    ...
    return (dists[order], embeds[order], [selections[i] for i in order])


def _entropy_map_from_mst(neighbor_embeds, query_embed, n_bins_nn):
    """
    Build the entropy map by running scipy MST on the t x t prefix of the pairwise
    distance matrix for t in [3, MAX_NEIGHBORS]. MST edge weights == H0 death times
    from a Rips filtration, so this matches the Ripser path exactly.
    """
    shifted = neighbor_embeds - query_embed[np.newaxis, :] + np.finfo(float).eps
    shifted = shifted / np.linalg.norm(shifted, axis=-1, keepdims=True)

    diff = shifted[:, None, :] - shifted[None, :, :]
    ...
    for t in range(3, MAX_NEIGHBORS + 1):
        ...
        if row_idx >= n_bins_nn:
            break
        mst = minimum_spanning_tree(D[:t, :t])
        weights = mst.data
        if weights.size == 0:
            continue
        h, _ = np.histogram(weights, bins=H0_BIRTH_BINS)
        entropy_map[row_idx] += h.astype(np.float32)
    return entropy_map


def generate_single_map(fg, rng, mode, n_bins_nn):
    ...
    non_query_features = np.array(
        [f for f in range(MAX_FEATURES) if f not in query_feature_ids_set]
    )
    ...
    pool_map = {"subset": subset_pool, "superset": superset_pool, "mixed": mixed_pool}
    mode_to_pools = {
        "subset": ["subset"],
        "superset": ["superset"],
        "mixed": ["mixed"],
    ...
    pool_feature_sets = []
    for _pd, _pe, pool_sels in pools:
    ...
    entropy_map = _entropy_map_from_mst(neighbor_embeds, query_embed, n_bins_nn)
...
def _init_worker(max_features, d, base_seed):
    global _FG
    _FG = FeaturesGenerator(max_features=max_features, d=d, seed=base_seed)


def _worker(task):
    i, base_seed, mode, n_bins_nn = task
    rng = np.random.default_rng(base_seed + i)
    result = generate_single_map(_FG, rng, mode=mode, n_bins_nn=n_bins_nn)
    return i, result
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fast entropy-map generator (MST + multiprocessing)")
    parser.add_argument("--n-maps", type=int, default=15000)
    ...
    parser.add_argument("--n-neighbour-bins", type=int, default=DEFAULT_N_BINS_NN)
    parser.add_argument(
        "--n-workers",
        type=int,
        default=0,
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Tasks dispatched per imap_unordered batch",
    )
    parser.add_argument("--upload-s3", action="store_true")
    parser.add_argument("--s3-bucket", type=str, default=S3_BUCKET)
    parser.add_argument("--s3-prefix", type=str, default=S3_PREFIX)
...
def main():
    ...
    maps_mmap = np.lib.format.open_memmap(
        str(maps_path), mode="w+", dtype=np.float32,
        shape=(n, args.n_neighbour_bins, N_BINS_H0),
    )
    labels_mmap = np.lib.format.open_memmap(
        str(labels_path), mode="w+", dtype=np.int8,
        shape=(n, MAX_NEIGHBORS, MAX_FEATURES),
    )
    ...
    n_workers = args.n_workers if args.n_workers > 0 else max(1, (os.cpu_count() or 2) - 1)
    print(f"Spawning {n_workers} worker processes")

    tasks = [
        (i, args.seed, MODES[i % len(MODES)], args.n_neighbour_bins)
        for i in range(n)
    ]
    ...
    with Pool(
        ...
        initargs=(MAX_FEATURES, EMBEDDING_DIM, args.seed),
    ) as pool:
        for i, result in tqdm(
            pool.imap_unordered(_worker, tasks, chunksize=args.chunksize),
            total=n, desc="Generating maps",
        ):
